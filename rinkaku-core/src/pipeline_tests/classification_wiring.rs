...
// ADR 0065: a whole-file deletion must report every base-side symbol
// as removed (alongside the existing `SkipReason::Deleted` entry),
// instead of vanishing from `report.removed` entirely.
#[test]
fn should_report_all_base_symbols_as_removed_when_whole_file_is_deleted() {
    let diff = "\
diff --git a/src/doomed.rs b/src/doomed.rs
deleted file mode 100644
index 4b825dc..0000000
--- a/src/doomed.rs
+++ /dev/null
@@ -1,7 +0,0 @@
-fn doomed_helper() -> i32 {
-    42
-}
-
-fn doomed_other() {
...
";
    ...
    let read_base_file = fake_reader(HashMap::from([("src/doomed.rs", base_source)]));
    ...
    let expected_removed = vec![
        RemovedSymbol {
            name: "doomed_helper".to_string(),
            kind: crate::extract::SymbolKind::Function,
            path: "src/doomed.rs".to_string(),
            signature: "fn doomed_helper() -> i32".to_string(),
        },
        RemovedSymbol {
            name: "doomed_other".to_string(),
            kind: crate::extract::SymbolKind::Function,
            path: "src/doomed.rs".to_string(),
            signature: "fn doomed_other()".to_string(),
        },
    ];
    assert_eq!(expected_removed, report.removed);
    let expected_skipped = vec![crate::render::SkippedFile {
        path: "src/doomed.rs".to_string(),
        reason: crate::render::SkipReason::Deleted,
    }];
    assert_eq!(expected_skipped, report.skipped);
    assert_eq!(Vec::<FileReport>::new(), report.files);
}

// ADR 0065's exclusion rules: a deleted file that the live pipeline
// would not analyze (unsupported language, generated, binary) must
// contribute nothing to `removed` either.
#[test]
fn should_leave_removed_empty_when_deleted_file_language_is_unsupported() {
    let diff = "\
...
";
    let read_file = fake_reader(HashMap::new());
    let read_base_file = fake_reader(HashMap::from([("notes.txt", "some text\n")]));
...
#[test]
fn should_leave_removed_empty_when_deleted_file_is_in_generated_paths() {
    let diff = "\
...
";
    let read_file = fake_reader(HashMap::new());
    let read_base_file = fake_reader(HashMap::from([(
        "src/generated.rs",
        "fn generated_fn() {}\n",
    )]));
    let generated_paths = HashSet::from(["src/generated.rs".to_string()]);
    ...
    assert_eq!(Vec::<RemovedSymbol>::new(), report.removed);
    // The skip entry keeps `Deleted` (checked before the generated-paths
    // check, ADR 0065: "removed a file" outranks "it was generated").
    let expected_skipped = vec![crate::render::SkippedFile {
...
#[test]
fn should_leave_removed_empty_when_deleted_base_content_carries_a_generated_marker() {
    let diff = "\
...
";
    let read_file = fake_reader(HashMap::new());
    let read_base_file = fake_reader(HashMap::from([("src/proto.rs", base_source)]));
